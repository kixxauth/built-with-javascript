...
export default class ObjectManagementClient {
    ...
    static fromConfig(logger, config) {
        const objectServiceEndpoint = config.objectService.getEndpoint();
        const objectServiceScope = config.objectService.getScope();
        const objectServiceToken = config.objectService.getToken();

        return new ObjectManagementClient({
            logger,
    ...
    constructor(options) {
        ...
        this.#logger = logger.createChild({ name: 'ObjectManagementClient' });
